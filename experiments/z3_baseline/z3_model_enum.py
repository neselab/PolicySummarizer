...
Claude 4 Sonnet, five candidates, and one syntax-repair attempt. All default
...
except ImportError:  # Progress rendering is optional.
...
DEFAULT_QUACKY_PATH = REPO_ROOT / "policysummarizer" / "quacky" / "quacky.py"
...
def run_command(command: list[str], *, cwd: Path, timeout: int) -> subprocess.CompletedProcess[str]:
    """Run a command and retain both output streams for the experiment trace."""
    try:
